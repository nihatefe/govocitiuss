 ...
 */
CoreGovocitos::CoreGovocitos(): interfazGovocitos(new InterfazGovocitos()),modeloGovocitos(new ModulosGovocitos())
{
    ...
    cout << "comprobando conectividad con base de datos ..." << endl;
...
/**
 Destructor, destruye los objetos de InterfazGovocitos y ModulosGovocitos.
 */
...
/**
 */
bool comprueba_ficheros_clasificador(const string &fichClasificEst, const string &fichClasificCl, const string &fichMediaDesvia) {
	string ruta;
	FILE  *f;


	ruta = Utiles::getClasificadorFile(fichClasificEst);
	//cout << "ruta=" << ruta << endl;

	f = fopen(ruta.c_str(), "r");
	if(! f) {
		fprintf(stderr, "error abriendo el archivo %s: %s: usando archivo por defecto\n", ruta.c_str(), strerror(errno));
		return(false);
	}
	fclose(f);

	ruta = Utiles::getClasificadorFile(fichClasificCl);
	f = fopen(ruta.c_str(), "r");
	if(! f) {
		fprintf(stderr, "error abriendo el archivo %s: %s: usando archivo por defecto\n", ruta.c_str(), strerror(errno));
		return(false);
	...
	ruta = Utiles::getClasificadorFile(fichMediaDesvia);
	f = fopen(ruta.c_str(), "r");
	if(! f) {
		fprintf(stderr, "error abriendo el archivo %s: %s: usando archivo por defecto\n", ruta.c_str(), strerror(errno));
		return(false);
...
void CoreGovocitos::clasificacionEstClAutomatica()
{
	...
	string  especie = imgActual->getSpecieImage ();
	string  fichClasificCl = especie + "_mejor_svm_nucleos.dat";
	string  fichClasificEst = especie + "_mejor_svm_estados.dat";
	string  fichMediaDesvia = especie + "_med_desv.dat";

	if(comprueba_ficheros_clasificador(fichClasificEst, fichClasificCl, fichMediaDesvia)) {
		clasificador = new Clasificador(Utiles::getClasificadorFile(fichClasificEst),
		                                Utiles::getClasificadorFile(fichClasificCl),
		                                Utiles::getClasificadorFile (fichMediaDesvia));
	} else {
		clasificador = new Clasificador(Utiles::getClasificadorFile("mz_mejor_svm_estados.dat"),
		                                Utiles::getClasificadorFile("mz_mejor_svm_nucleos.dat"),
		                                Utiles::getClasificadorFile("mz_med_desv.dat"));
        string mensaje = "Classifier files not found for species " + especie + "\n Using classifier for species Merluccius Merluccius ...";
        string titulo = "Classifier files not found";
        Dialogos::dialogoAdvertencia(mensaje, titulo);
	}
...
/**
 */
bool CoreGovocitos::creaPatronesClasificador(const vector<string> &imagen, const vector<string> &ficheroXml) {
	string  rutaImagen, nombreImagen, rutaXml, rutaSftp;
	Image *img;
	HttpsConexion *conexionImagenes;
	DataImage *imagenDescarga = NULL;
	ListadoCelulas  *listCell;
	PointList *puntos;
	vector <int>  *cx, *cy;
	double  *resul;
	int  numCelulas, numPuntos, i, j, k, l, anchoImg, altoImg, min_x, max_x, min_y, max_y, numImagenes;


	DialogoBarraProgres dProgres("Processing images");
	dProgres.setEstadoBarraProgreso(0.0);
	dProgres.setPercentText(0.0);
	dProgres.ejectuaDialogoProgreso();
	Utiles::actualizarInterfaz();

	numImagenes = imagen.size();
	conexionImagenes = modeloGovocitos->getConexionBD();
	rutaSftp = modeloGovocitos->getConfiguracion()->getDatosPreferencias()->getPathSFTP();

	int  z[numImagenes], numImagenesSelec = numImagenes;

	for(i = 0; i < numImagenes; i++) {
		z[i] = i;
	}
	if(numImagenes > N_IMAGENES_MAX) {
		int t, n;

		for(i = 0, n = numImagenes - 1; i < n; i++) {
			j = (float) numImagenes*rand()/RAND_MAX;
			t = z[i]; z[i] = z[j]; z[j] = t;
		}
		/*for(i=0;i<numImagenes; i++) {
			printf("%i ", z[i]);
		...
		printf("\n");*/
		numImagenesSelec = N_IMAGENES_MAX;
	}
	printf("numImagenesSelec=%i\n", numImagenesSelec);

	numPatrones = 0;
	string  nombreXml, claseCelula, estadoCelula;
	double  porcProgreso;

	for(l = 0; l < numImagenesSelec; l++) {
		i = z[l];

		if(! conexionImagenes->descargaFicheroXml(ficheroXml[i])) { // comprueba si hay fichero Xml
			continue;
		}

		Utiles::extrae_ruta_fichero(imagen[i], rutaImagen, nombreImagen);
		printf("procesando imagen %i/%i %s ...\n", l+1, numImagenesSelec, nombreImagen.c_str());

		// comprueba si la imagen está en la caché de govocitos
		string  rutaImagenCache = Utiles::direccionCache(nombreImagen);
		//cout << "rutaImagenCache=" << rutaImagenCache << endl;

		if(! Fichero::compruebaFich(rutaImagenCache)) {  // no está en la caché
			imagenDescarga = new DataImage();
			imagenDescarga->setRutaImg(rutaImagen);
			imagenDescarga->setNombreImg(nombreImagen);

			if(! conexionImagenes->descargaImagen(imagenDescarga)) { // comprueba si hay imagen
				continue;
			}
			rutaImagen.assign(Utiles::direccionCache(imagenDescarga->getNombreImgCompleto()));
			delete imagenDescarga;
		} else {
			//printf("la imagen %s ya está en la cache\n", imagen[i].c_str());
			rutaImagen = rutaImagenCache;
		}

		nombreXml = Utiles::extrae_fichero(ficheroXml[i]);
		rutaXml = Utiles::direccionCache(nombreXml);
		//cout << "ficheroXml=" << ficheroXml[i] << " nombreXml= " << nombreXml << " rutaXml= " << rutaXml << endl;

		listCell = new ListadoCelulas();
		Fichero::leerFichXml(rutaXml, *listCell);

		//Guardamos la imagen actual en bmp para que la lea el clasificador
		Glib::RefPtr<Gdk::Pixbuf>  imagenCelulas = Gdk::Pixbuf::create_from_file(rutaImagen);
		rutaImagen = rutaImagen + ".bmp";
		imagenCelulas->save(rutaImagen, "bmp");

		img = read_img (rutaImagen.c_str() );
		anchoImg = get_num_cols(img);
		altoImg = get_num_rows(img);

		numCelulas = listCell->getNumCelulas();
		//printf("imagen %s: numCelulas= %i\n", nombreImagen.c_str(), numCelulas);

		// calcula los vectores de características de las células
		for(j = 0; j < numCelulas; j++)
		{
			cx = listCell->getCelula(j)->getBordeCellX();
			cy = listCell->getCelula(j)->getBordeCellY();
			...
			//cout << "célula " << j << " min_x=" << min_x << " " << max_x << " min_y= " << min_y << " " << max_y << endl;
			if(min_x < MARXE_BORDE_ENTRENA || max_x > anchoImg - MARXE_BORDE_ENTRENA || min_y < MARXE_BORDE_ENTRENA || max_y > altoImg - MARXE_BORDE_ENTRENA) {
				//cout << "célula descartada" << endl;
				continue;
			}
			claseCelula = listCell->getCelula(j)->getClaseCelula();
			if(claseCelula != "cn" && claseCelula != "sn") {
				fprintf(stderr, "patrón %i de imagen %s descartado por tener clase \"%s\" invalida\n", j, imagen[i].c_str(), claseCelula.c_str());
				continue;
			}

			estadoCelula = listCell->getCelula(j)->getEstadoCelula();
			if(estadoCelula != "ac" && estadoCelula != "hid" && estadoCelula != "vit" && estadoCelula != "atre") {
				fprintf(stderr, "patron %i de imagen %s descartado por tener estado \"%s\" invalido\n", j, imagen[i].c_str(), estadoCelula.c_str());
				continue;
			}

			clase.push_back(claseCelula);
			estado.push_back(estadoCelula);
			...
			{
				puntos->point[k] = alloc_point((*cy)[k], (*cx)[k]);
			}
			puntos->type = GEN_VALID;

			resul = textureLBPColor ( img, puntos);
			patron.push_back(resul);
			...
			if(++numPatrones == N_PATRONES_MAX) {
				break;
			}
			//printf("numPatrones= %i\n", numPatrones);
		}
		free_img(img); free(img);

		delete listCell;
		//cout << "imagen " << i << " numCelulas=" << numCelulas << " n_patrones=" << patron.size() << endl;

		if(numPatrones == N_PATRONES_MAX) {
			break;
		}

		porcProgreso = (float) l/numImagenesSelec;
...
/**
 Entrena el clasificador de estados y clases
 */
void CoreGovocitos::entrenamientoEstCl()
{
	ConexionBD *conMysql = modeloGovocitos->getConexionMySql();
	vector<string>  especies;
	int  especieSelec;

	// consulta de las especies disponibles
	conMysql->consultaEspecies(especies);
	int  numEspecies = especies.size();
	if(! numEspecies) {
	    Dialogos::dialogoError("No species available. Training aborted", "Error connecting to Database");
	    return;
	}

	// selección de especie en ventana de especie
	if(! ventanaSeleccionEspecie(numEspecies, especies, especieSelec)) {
		return;  // se pulsó el botón cancelar
	}
	nombreEspecie = especies[especieSelec];
	cout << "especie seleccionada= " << especieSelec << " " << nombreEspecie << endl;

	// consulta de las imágenes disponibles de la especie seleccionada
	vector<string>  imagen, ficheroXml;
	conMysql->consultaImagenesEspecie(nombreEspecie, imagen, ficheroXml);
	cout << "No. imagenes=" << imagen.size() << " de la especie "<< nombreEspecie << " numFichXml= " << ficheroXml.size() << endl;

	if(! imagen.size() || ! ficheroXml.size()) {
		Dialogos::dialogoError("No images found for species " + nombreEspecie, "Error");
		return;
	}

	// crea los patrones para el clasificador
	if(! creaPatronesClasificador(imagen, ficheroXml)) {
		Dialogos::dialogoError("No cell found for species " + nombreEspecie, "Error");
		return;
	}

	// entrenamiento con los patrones creados
	entrenamiento();

	// envia ficheros de clasificador a base de datos: conMysql ya esta del principio de este metodo
	SftpConexion *conSftp = modeloGovocitos->getConexionSftp();
	if(! conMysql->enviaFicherosClasificador(nombreEspecie, fich, conSftp)) {
        Dialogos::dialogoError("Unable to send classifier files", "Error");
	} else {
        Dialogos::dialogoInformacion("Training finished correctly", "Information");
	}
}

/**
 Actualiza el clasificador de estados y clases desde la base de datos
 */
void CoreGovocitos::actualizacionClasificador()
{
	ConexionBD *conMysql;
	vector<string>  especies;
	...
	conMysql = modeloGovocitos->getConexionMySql();
	conMysql->consultaEspecies(especies);
	HttpsConexion  *conHttps = modeloGovocitos->getConexionBD();
	if(! conMysql->descargarFicherosClasificador(especies, conHttps)) {
		Dialogos::dialogoError("Error downloading classifiers", "Error");
	} else {
		Dialogos::dialogoInformacion("The classifiers were correctly downloaded", "Information");
	}
	printf("CoreGovocitos::actualizacionClasificador termino correctamente\n");
}
