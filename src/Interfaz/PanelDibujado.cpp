...
{	
	...
	if(surfaceCelulas && (cX >= 0) && (cY >= 0) && (cX <= anchoRedimen) && (cY <= altoRedimen))
	{
		if(event->button == 1)
		{
			...
			else if(panelDer->getPagBlocnotas() == 3)
			{
				panelBDetectionButtons(event);}
		}
...
void PanelDibujado::panelBDetectionButtons(GdkEventButton *event)
{
	...
	if(panelDer->getDrawcircle())
		{
			capaCelula->Drawcircle((event->x - desplazaX), 
				                   (event->y - desplazaY));
...
void PanelDibujado::panelEdicionButtons(GdkEventButton *event)
{
		...
		if(capaCelula->getNumsCelulaSeleccionada())
		{
			...
			{	
				...
				else
				{
					...
					panelDer->setSensitiveDrawcircle(true);
	...
	else if(panelDer->getNotUseAreaImage2())
	{
		capaCelula->destruyeDatos();
	...
	{	
		...
		{	
			if(!capaCelula->getSeccionBorrada())
			{
				...
				/*if(panelDer->getNotUseAreaImage2())
				{
					capaCelula->addCellToListCells(true);
				}
				else
