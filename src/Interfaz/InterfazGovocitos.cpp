...
void InterfazGovocitos::destruyeDatos()
{
	...
	if(panelDatos.get_child2())
	{
		...
		{
			panelProp->setSentiviePrintData2(false);
...
void InterfazGovocitos::activaBDetection()
{
	//Si hay algun panel de datos se destruira
	destruyeDatos();
	...
	//Establece la vista del panel
	panelProp->setPagBlocNotas(3);
	panelProp->activaCapaCelulas();
	panelProp->show();

	
	panelProp->setLabelPanel("<b>BDetection Panel</b>");
}
...
/**
 Lanza el entrenamiento del clasificador
 */
void InterfazGovocitos::entrenamiento()
{
	driverGovocitos->entrenamientoEstCl();
...
/**
 Actualiza el clasificador
 */
void InterfazGovocitos::actualizacionClasificador()
{
	driverGovocitos->actualizacionClasificador();
	//menBarPrincipal->setSensitiveOpcMenu("SubMenuClasificacion", true);
	...
	switch(panel)
	{
		...
		case 2:
		{
			...
			if(datosD->calculoDiametrosClass())
			{
				...
				panelProp->setSentiviePrintData2(true);
			}
