...
void MenuBarraPrincipal::setSensibleOpcionesBasicas()
{
	...
	mPrincipal->get_action("SubMenuBDetection")->set_sensitive(true);
...
void MenuBarraPrincipal::estableceMenuPosiconInicial()
{
	...
	mPrincipal->get_action("SubMenuBDetection")->set_sensitive(false);
	mPrincipal->get_action("SubMenuRehacer")->set_sensitive(false);
	mPrincipal->get_action("SubMenuDeshacer")->set_sensitive(false);
	...
	mPrincipal->get_action("SubMenuEntrenamiento")->set_sensitive(true);
	mPrincipal->get_action("SubMenuActualizacionClasificador")->set_sensitive(true);
...
void MenuBarraPrincipal::InicializaMenuBarraPrincipal()
{
	...
	//BURASI
	mPrincipal->add(Gtk::Action::create("SubMenuBDetection",Gtk::Stock::PROPERTIES,
										"BDetection Panel"),
					Gtk::AccelKey("<control>Y"),
					sigc::mem_fun(*interfazPrincipal, &InterfazGovocitos::activaBDetection));
	...
	mPrincipal->add(Gtk::Action::create("SubMenuEntrenamiento",Gtk::Stock::DND,
										"Train classifier"),
					Gtk::AccelKey("<control><shift>T"),
					sigc::mem_fun(*interfazPrincipal, &InterfazGovocitos::entrenamiento));
	// Submenu actualizacion clasificador
	mPrincipal->add(Gtk::Action::create("SubMenuActualizacionClasificador",Gtk::Stock::DND,
										"Download classifiers"),
					Gtk::AccelKey("<control><shift>D"),
					sigc::mem_fun(*interfazPrincipal, &InterfazGovocitos::actualizacionClasificador));
...
void MenuBarraPrincipal::CreaUIManagerMenu()
{
	...
	Glib::ustring ui_info =
		...
		//	"			<menuitem action = 'SubMenuGuardarMascara' />"
			...
			"			<menuitem action = 'SubMenuBDetection' />"
			"	   </menu>"
...
/*			"	   <menu action = 'MenuAvanzado'>"
			...
			"			<menuitem action = 'SubMenuEntrenamiento' />"
			"			<menuitem action = 'SubMenuActualizacionClasificador' />"
			"	   </menu>"
			...
			"			<separator/>"
			"			<toolitem action = 'SubMenuBDetection' />"
			"   </toolbar>"
			"</ui>";
