...
bool ConexionBD::obtenerIndividuo(vector<ListadoIndividuo> &li)
{
    ...
    if(conn.connect(dataBase.c_str(), urlDataBase.c_str(), userDataBase.c_str(), passDataBase.c_str()))
	{
		...
		sentencia = sentencia + "ORDER BY individuos.codigo_campanha, individuos.codigo_especie, individuos.codigo_muestreo, individuos.codigo_individuo, imagenes.codigo_imagen";

		//sentencia = sentencia + "ORDER BY individuos.codigo_individuo, imagenes.codigo_imagen"; //vello borrar
...
/**
 Descarga los ficheros del clasificador (para todas las especies) desde la base de datos del servidor (por Sftp)
 @param especie, vector de especies para las cuales se descargan los ficheros del clasificador
 @param conex, objeto de la HttpsConexion, contiene los datos de conexion para realizar la descarga de ficheros al servidor.
 @return bool, devuelve true si la consulta es correcta false en caso contrario.
 */
bool ConexionBD::descargarFicherosClasificador(vector<string> &especie, HttpsConexion *conex)
{
	mysqlpp::Connection conn(false);

    if(conn.connect(dataBase.c_str(), urlDataBase.c_str(), userDataBase.c_str(), passDataBase.c_str()))
	{
		string sentSelect = "select path_datos_clasificador from especies";
        ...
        if(! res.empty())
		{
			string  rutaOrigen, rutaDestino, fich[3];
			int  i, numEspecies = especie.size();

			for(i = 0; i < numEspecies; i++) {
				//rutaOrigen = (string) res[i]["path_datos_clasificador"] + especie;
				rutaOrigen = (string) res[i]["path_datos_clasificador"];

				fich[0] = especie[i] + "_mejor_svm_nucleos.dat";
				fich[1] = especie[i] + "_mejor_svm_estados.dat";
				fich[2] = especie[i] + "_med_desv.dat";

				if(! conex->descargarFicherosClasificador(rutaOrigen, especie[i], fich)) {
					return false;
				}
			}
		}
...
/**
 Envía los archivos del clasificador a la base de datos
 @param especie, nombre de la especie para la que se actualiza los ficheros del clasificador
 @param fich, vector con los nombres de los ficheros del clasificador
 @param conex, objeto de la SftpConexion, contiene los datos de conexion para realizar
 la subida de ficheros al servidor.
 @return bool, devuelve true si la consulta es correcta false en caso contrario.
 */
bool ConexionBD::enviaFicherosClasificador(const string &especie, const string *fich, SftpConexion *conex)
{
	mysqlpp::Connection conn(false);
    ...
    if(conn.connect(dataBase.c_str(), urlDataBase.c_str(), userDataBase.c_str(), passDataBase.c_str()))
	{
        ...
        if(! res.empty())
		{
			string  rutaSftp = (string) res[0]["path_datos_clasificador"];
			//Subimos el fichero con el sftp
			if(! conex->subirFicherosClasificadorServidor(rutaSftp, fich)) {
				return false;
			}
		}
	}
...
/**
 Consulta las especies disponibles en el servidor.
 @param especies, referencia al vector de string que contiene las especies.
 @return bool, devuelve true si la consulta es correcta false en caso contrario.
 */
bool ConexionBD::consultaEspecies(vector<string> &especies)
{
	mysqlpp::Connection conn(false);
    ...
    if(conn.connect(dataBase.c_str(), urlDataBase.c_str(), userDataBase.c_str(), passDataBase.c_str()))
	{
		...
		sentencia = "select * from especies";
        ...
        if(!res.empty())
		{
			for (size_t i = 0; i < res.num_rows(); ++i)
			{
				especies.push_back((string) res[i]["codigo_especie"]);
			}
		}
	}
	else
...
/**
 Consulta las especies disponibles en el servidor.
 @param especie, string con el código de la especie
 @param individuosEspecie, vector con los códigos de los individuos de la especie
 @return bool, devuelve true si la consulta es correcta false en caso contrario.
 */
bool ConexionBD::consultaIndividuosEspecie(string especie, vector<string> &individuosEspecie)
{
	mysqlpp::Connection conn(false);
    ...
    if(conn.connect(dataBase.c_str(), urlDataBase.c_str(), userDataBase.c_str(), passDataBase.c_str()))
	{
		...
		sentencia = "select * from individuos where codigo_especie = \"" +  especie + "\"";
        ...
        if(!res.empty())
		{
			for (size_t i = 0; i < res.num_rows(); ++i)
			{
				individuosEspecie.push_back((string) res[i]["codigo_individuo"]);
			}
		}
	}
	else
...
/**
 Consulta las imágenes de una especie disponibles en el servidor con fichero XML.
 @param especie, string con el código de la especie
 @param imagen, vector con las rutas de las imágenes de la especie
 @param ficheroXML, vector con las rutas de los ficheros XML de las imágenes
 @return bool, devuelve true si la consulta es correcta false en caso contrario.
 */
bool ConexionBD::consultaImagenesEspecie(string especie, vector<string> &imagen, vector<string> &ficheroXML)
{
	mysqlpp::Connection conn(false);
    ...
    if(conn.connect(dataBase.c_str(), urlDataBase.c_str(), userDataBase.c_str(), passDataBase.c_str()))
	{
		...
		sentencia = "select path_original,path_ovocitos from imagenes where codigo_especie = \"" +  especie + "\" AND path_ovocitos IS NOT NULL";
        ...
        if(!res.empty())
		{
			for (size_t i = 0; i < res.num_rows(); ++i)
			{
				imagen.push_back((string) res[i]["path_original"]);
				ficheroXML.push_back((string) res[i]["path_ovocitos"]);
			}
		}
	}
	else
