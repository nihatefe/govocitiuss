...
void DibujadoCelula::Drawcircle(const int &cX, const int &cY)
{	
 
  /*borrarCelulaNueva();
  crCircle->save();
  crCircle->arc(cX, cY, (1 * zoomCelulas), 0, 2 * M_PI); // full circle
  crCircle->set_source_rgba(ediCelR, ediCelG, ediCelB, 1); // partially translucent
  crCircle->fill_preserve();
  crCircle->restore();  // back to opaque black
  crCircle->stroke();*/

	//Ancho de la linea de dibujo
	crCircle->set_line_width(grosLinea * zoomCelulas * 1.5);
	
	//Color del punto
	crCircle->set_source_rgba(ediCelR, ediCelG, ediCelB, 1);
	
	//Dibujamos un circulo con el centro cX,cY
	crCircle->arc(cX, cY, (1 * zoomCelulas), 0, 2 * M_PI);
