...
bool SftpConexion::subirFicheroServidor(const string &rutaXml, ImageCells &img)
{
    ...
    if(auth_pw)
	{
        ...
        if(rc)
		{
		    ...
		    cout << "user=" << userSftp << " passwd=" << passSftp << endl;
            systemshutdownFTP(session, sock);
...
/**
 Sube un fichero a la base de datos.
 @param ruta, array de strings con las rutas de los ficheros del clasificador.
 @return bool, true si se sube el fichero con exito, false en caso contrario.
 */
bool SftpConexion::subirFicherosClasificadorServidor(const string &rutaSftp, const string *fich)
{
	struct hostent * hostent;
    int sock, auth_pw = 1;
    struct sockaddr_in sin;
	...
	string  fichSftp;
	for(int i = 0; i < 3; i++) {
		fichSftp = rutaSftp + Utiles::extrae_fichero(fich[i]);
		//cout << "fichLocal=" << fich[i] << endl;
		//cout << "fichRemoto=" << fichSftp << endl;
	    if(! envioDeDatos(session, sock, sftp_session, rc, fich[i], fichSftp))
		{
			fprintf(stderr, "Unable to send file %s\n", fichSftp.c_str());
			return false;
	    }
		//printf("enviado fichero %s\n", fich[i].c_str());
    	percetUpload = i/3.;
		dProgres.setEstadoBarraProgreso(percetUpload);
		dProgres.setPercentText(percetUpload);
		//Actualizamos la ventana
...
/**
*/
void SftpConexion::compruebaError(LIBSSH2_SFTP *sftp_session) {
	unsigned long errno1 = libssh2_sftp_last_error(sftp_session);
	switch(errno1) {
		case LIBSSH2_FX_OK:
			printf("LIBSSH2_FX_OK\n");
			break;
		case LIBSSH2_FX_EOF:
			printf("LIBSSH2_FX_EOF\n");
			break;
		case LIBSSH2_FX_NO_SUCH_FILE:
			printf("LIBSSH2_FX_NO_SUCH_FILE\n");
			break;
		case LIBSSH2_FX_PERMISSION_DENIED:
			printf("LIBSSH2_FX_PERMISSION_DENIED\n");
			break;
		case LIBSSH2_FX_FAILURE:
			printf("LIBSSH2_FX_FAILURE\n");
			break;
		case LIBSSH2_FX_BAD_MESSAGE:
			printf("LIBSSH2_FX_BAD_MESSAGE\n");
			break;
		case LIBSSH2_FX_NO_CONNECTION:
			printf("LIBSSH2_FX_NO_CONNECTION\n");
			break;
		case LIBSSH2_FX_CONNECTION_LOST:
			printf("LIBSSH2_FX_CONNECTION_LOST\n");
			break;
		case LIBSSH2_FX_OP_UNSUPPORTED:
			printf("LIBSSH2_FX_OP_UNSUPPORTED\n");
			break;
		case LIBSSH2_FX_INVALID_HANDLE:
			printf("LIBSSH2_FX_INVALID_HANDLE\n");
			break;
		case LIBSSH2_FX_NO_SUCH_PATH:
			printf("LIBSSH2_FX_NO_SUCH_PATH\n");
			break;
		case LIBSSH2_FX_FILE_ALREADY_EXISTS:
			printf("LIBSSH2_FX_FILE_ALREADY_EXISTS\n");
			break;
		case LIBSSH2_FX_WRITE_PROTECT:
			printf("LIBSSH2_FX_WRITE_PROTECT\n");
			break;
		case LIBSSH2_FX_NO_MEDIA:
			printf("LIBSSH2_FX_NO_MEDIA\n");
			break;
		case LIBSSH2_FX_NO_SPACE_ON_FILESYSTEM:
			printf("LIBSSH2_FX_NO_SPACE_ON_FILESYSTEM\n");
			break;
		case LIBSSH2_FX_QUOTA_EXCEEDED:
			printf("LIBSSH2_FX_QUOTA_EXCEEDED\n");
			break;
		case LIBSSH2_FX_UNKNOWN_PRINCIPLE:
			printf("LIBSSH2_FX_UNKNOWN_PRINCIPLE\n");
			break;
		case LIBSSH2_FX_LOCK_CONFlICT:
			printf("LIBSSH2_FX_LOCK_CONFlICT\n");
			break;
		case LIBSSH2_FX_DIR_NOT_EMPTY:
			printf("LIBSSH2_FX_DIR_NOT_EMPTY\n");
			break;
		case LIBSSH2_FX_NOT_A_DIRECTORY:
			printf("LIBSSH2_FX_NOT_A_DIRECTORY\n");
			break;
		case LIBSSH2_FX_INVALID_FILENAME:
			printf("LIBSSH2_FX_INVALID_FILENAME\n");
			break;
		case LIBSSH2_FX_LINK_LOOP:
			printf("LIBSSH2_FX_LINK_LOOP\n");
			break;
		default:
								 ...
								 int &rc, const string &pathXml, const string &nombreXml)
{
    ...
    do {
		...
		{
            fprintf(stderr, "Unable to open file with SFTP: error in libssh2_sftp_open\n");
            compruebaError(sftp_session);
            cout << "auxPathSftp=" << auxPathSftp << endl;
            systemshutdownFTP(session, sock);
			return false;
        }
    } while (!sftp_handle);
    ...
    if (rc < 0)
	{
        fprintf(stderr, "Unable to open file with SFTP: error in libssh2_sftp_write\n");
        compruebaError(sftp_session);
		systemshutdownFTP(session, sock);
		return false;
	}
