...
int progress_callback(void *clientp, double dltotal, double dlnow, double ultotal, double ulnow)
{
	...
	if(!dProgres)
	{
		...
		dProgres = new DialogoBarraProgres("Downloading Image");
	}
...
/**
 Descarga un fichero xml del servidor que contiene las imagenes con celulas.
 @param rutaXml, string con la ruta completa del fichero Xml.
 @return bool, verdadero si la descarga es correcta, falso en caso contrario.
 */
bool HttpsConexion::descargaFicheroXml(const string &rutaXml)
{
	CURL *curl;
	CURLcode res;
	FILE *destFile(0);
	...
	//Comprobamos el tipo de conexion a realizar
	urlFinal = urlPagina + "/" + rutaXml;
	//cout << "urlFinal=" << urlFinal << endl;
	...
	if(curl)
	{
		...
		// Tell libcurl where to write the file
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, writeFunction);
		string  nombreXml = Utiles::extrae_fichero(urlFinal);
		pathsistema = Utiles::direccionCache(nombreXml);
...
/**
 Descarga un fichero xml del servidor que contiene las imagenes con celulas.
 @param rutaOrigen, string con la ruta desde que se descarga el fichero del clasificador
 @param especie, string con la especie a la que está asociado el clasificador
 @param fich, array de string con los nombres de los archivos del clasificador
 @return bool, verdadero si la descarga es correcta, falso en caso contrario.
 */
bool HttpsConexion::descargarFicherosClasificador(const string &rutaOrigen, const string &especie, const string *fich)
{
	CURL *curl;
	CURLcode res;
	FILE *destFile(0);
	string urlFinal, rutaConfig, rutaAlmac;
	int  i;
	...
	if(curl)
	{
		rutaConfig = Utiles::obtenerDireccionClasificador();
		//cout << "rutaConfig=" << rutaConfig << endl;

		for(i = 0; i < N_FICH_CLASIF; i++) {
			urlFinal = urlPagina + "/" + rutaOrigen + fich[i];
			//cout << "urlFinal=" << urlFinal << endl;
			...
			// Tell libcurl where to write the file
			curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, writeFunction);
			rutaAlmac = rutaConfig + "/" + fich[i];
			//cout << "descargando en " << rutaAlmac << endl;
			destFile = fopen(rutaAlmac.c_str(),"wb");
			curl_easy_setopt(curl, CURLOPT_WRITEDATA, destFile);
