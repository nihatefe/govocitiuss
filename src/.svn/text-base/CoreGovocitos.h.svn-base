 ...
 */
	...
	// usamos como máximo 50 imágenes: deberían tener en torno a 1000 células (o patrones)
	#define N_IMAGENES_MAX 50

	extern vector<double *>  patron;
	extern vector<string>  clase;
	extern vector<string>  estado;
	extern vector<string>  especies;
	extern string  fich[N_FICH_CLASIF];
	extern string  nombreEspecie;
	extern int  especieSelec;
	extern int  numPatrones;

	extern void entrenamiento();
	...
	class CoreGovocitos
	{
		public:
			...
			bool creaPatronesClasificador(const vector<string> &imagen, const vector<string> &ficheroXml);
			void entrenamientoEstCl();
			void actualizacionClasificador();
